...
class FinesController extends ResourceController
{
    ...
    public function searchReturn()
    {
        if ($this->request->isAJAX()) {
            ...
            $returns = $this->loanModel
                ->select('members.*, items.*, fines.*, fines.id as fine_id, fines.deleted_at as fine_deleted, loans.*')
                ->join('members', 'loans.member_id = members.id', 'LEFT')
                ->join('items', 'loans.item_id = items.id', 'LEFT')
                ->join('fines', 'fines.loan_id = loans.id', 'INNER')
    ...
    {
        $returns = $this->loanModel
            ->select('members.*, items.*, fines.*, fines.id as fine_id, fines.deleted_at as fine_deleted, racks.name as rack, loans.*')
            ->join('members', 'loans.member_id = members.id', 'LEFT')
            ->join('items', 'loans.item_id = items.id', 'LEFT')
