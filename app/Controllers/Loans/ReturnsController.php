...
class ReturnsController extends ResourceController
{
    ...
    public function index()
    {
        ...
        if ($this->request->getGet('search')) {
            ...
            $loans = $this->loanModel
                ->select('members.*, members.uid as member_uid, items.*, fines.*, fines.id as fine_id, fines.deleted_at as fine_deleted, loans.*')
    ...
    {
        $loan = $this->loanModel
            ->select('members.*, members.uid as member_uid, items.*, fines.*, fines.id as fine_id, loans.*, loans.qr_code as loan_qr_code, item_stock.quantity as item_stock, racks.name as rack, categories.name as category')
            ->join('members', 'loans.member_id = members.id', 'LEFT')
            ->join('items', 'loans.item_id = items.id', 'LEFT')
    ...
    public function new()
    {
        ...
        $loans = $this->loanModel
            ->select('members.*, items.*, categories.name as category, loans.*')
            ->join('members', 'loans.member_id = members.id', 'LEFT')
            ->join('items', 'loans.item_id = items.id', 'LEFT')
            ->join('categories', 'items.category_id = categories.id', 'LEFT')
